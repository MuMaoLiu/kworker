...
function copy_uboot_source(){
    ...
    if [ -n "${KH_BIOS_ROOT}" ] && [ -d "${KH_BIOS_ROOT}" ]; then
        :
    elif [ -d "${SCRIPT_DIR}/../uboot_bios" ]; then
        KH_BIOS_ROOT="$(cd "${SCRIPT_DIR}/../uboot_bios" && pwd)"
    elif [ -d "${ROOT_DIR}/extension/uboot_bios" ]; then
        KH_BIOS_ROOT="${ROOT_DIR}/extension/uboot_bios"
    else
        echo "[kh_bios] ERROR: uboot_bios not found (set KH_BIOS_ROOT or sync to extension/uboot_bios)"
        exit 1
    fi
