...
**完整说明（合入主仓、编译、烧录、验证、移植、编译开关）见：[`../README.md`](../README.md)**
...
| `core/Kconfig` | `CONFIG_KH_BIOS`、`CONFIG_KH_BIOS_HOTKEY_F10` 定义 |
...
# 例：integrate_kh_bios.sh out/uboot/khdvk_rk3568_a/src_tmp device/board/{company}/{product}/uboot
```

脚本在 out 树中完成：

- 拷贝 `core/` + `include/` + 板级 `bios/` → `cmd/kh_bios/`
- `cmd/Makefile` 增加 `obj-$(CONFIG_KH_BIOS) += kh_bios/`
- `cmd/Kconfig` 增加 `source "cmd/kh_bios/Kconfig"`
- 打 `patches/rockchip_video_fb*.patch`

## 编译开关速查

| 层级 | 位置 | 说明 |
|------|------|------|
| Kconfig 定义 | `core/Kconfig` | `CONFIG_KH_BIOS`（depends on `DM_VIDEO`）、`CONFIG_KH_BIOS_HOTKEY_F10` |
| 板级打开 | `../uboot/configs/*_defconfig` | 3568A：`CONFIG_KH_BIOS=y` 等 |
| 链接 | out 树 `cmd/Makefile` | `obj-$(CONFIG_KH_BIOS) += kh_bios/` |
| 运行时核对 | `out/uboot/{product}/OBJ/.config` | `grep CONFIG_KH_BIOS` |

**defconfig 最低要求**（详见 [`../README.md#编译开关kconfig--defconfig`](../README.md#编译开关kconfig--defconfig)）：

| 配置项 | 说明 |
|--------|------|
| `CONFIG_KH_BIOS=y` | 启用 BIOS（总开关） |
| `CONFIG_DM_VIDEO=y` | HDMI（KH_BIOS 依赖） |
